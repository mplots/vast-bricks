  ...
  case "${command}" in
      ...
      case "${service_action}" in
        start)
          if [[ "${current}" == -* ]]; then
            ...
            COMPREPLY=($(compgen -W "postgres tor-proxy vast-api-test vast-api wiremock vast-portal" -- "${current}"))
          fi
          return
          ;;
        restart)
    ...
    logs)
      ...
      if [[ "${COMP_WORDS[2]}" == "clear" ]]; then
        ...
        COMPREPLY=($(compgen -W "clear postgres tor-proxy vast-api-test vast-api wiremock vast-portal" -- "${current}"))
      else
        COMPREPLY=()
      fi
      return
