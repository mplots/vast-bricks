 ...
 *
 * <p>They are the bank statement's three with the fee line between the turnovers and the sum, which is the one thing
 * a Stripe ledger has that a statement does not: a bank charges its fees as entries of their own, and Stripe deducts
 * them from the transactions they belong to. So the fees have to be stated separately for the sum to add up in front
 * of the reader.
 *
 * <p>The last line is what the balance moved by rather than a closing balance. Stripe knows the account's balance and
...
const summaryLines = (currency: StripeTransactionCurrencySummary, label: (id: string) => string): SummaryLine[] => {
  ...
  return [
    ...
    },
    {
      ...
      amount: `−${money(Math.abs(currency.fees))}`,
      label: label('stripe-transaction-fees'),
      colour: 'error.main'
    },
    {
      // What the three above come to, so it is ruled off from them, and signed already: a period that gave back more
      // than it took is a fact about the period rather than a direction of movement.
      key: `${currency.currency}:net`,
      amount: money(currency.net),
                        ...
                        {shown.map((transaction) => (
                          <TableRow key={transaction.id} hover>
                            ...
                            {/* Nothing deducted is no fee rather than a zero: Stripe takes its fee out of the
                                transaction it belongs to rather than out of all of them. */}
                            <TableCell sx={{ ...numericCell, color: 'text.secondary' }}>
                              {transaction.fee === null ? '—' : `−${formatAmount(transaction.fee)}`}
                            </TableCell>
