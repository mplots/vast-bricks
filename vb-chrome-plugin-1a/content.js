...
const LAST_RUN_KEY = 'vb_last_run_1a';
...
(async () => {
    ...
    const numberOfPages = document.querySelector('span.paginator__last')?.textContent.trim();
    for(let i=1; i<=numberOfPages; i++ ) {
        [...document.querySelectorAll('.paginator a')]
            .find(a => a.textContent.trim() === String(i))
            ?.click();

        console.log(`posting page ${i}`)

        await post('https://tool.vastbricks.com/api/1a')
    }
